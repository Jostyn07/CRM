...
// Ventana de chat de Wazzup (historial completo, incluye lo anterior a la conexión)
export async function getHistoryUrl(conversationId) {
  const { data, error } = await supabase.functions.invoke('whatsapp-iframe', { body: { conversation_id: conversationId } });
  if (error) throw new Error(await fnError(error, 'No se pudo abrir el historial'));
  return data.url;
}