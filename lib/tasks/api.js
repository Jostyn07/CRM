...
// ---------------------------------------------------------------
// Respuestas dentro de una tarea
// ---------------------------------------------------------------
const COMMENT_COLS = 'id, task_id, user_id, body, created_at, edited_at';
...
export async function addTaskComment(taskId, body) {
  const { data, error } = await supabase.from('task_comments').insert({ task_id: taskId, body }).select(COMMENT_COLS).single();
  if (error) {
    if (error.code === '42501') throw new Error('No puedes responder en esta tarea.');
    throw new Error(taskError(error));
  }
  return data;
}

export async function editTaskComment(id, body) {
  const { data, error } = await supabase.from('task_comments').update({ body }).eq('id', id).select(COMMENT_COLS).maybeSingle();
  if (error) throw new Error(taskError(error));
  if (!data) throw new Error('Solo el autor puede editar su respuesta.');
  return data;
}

export async function getCommentCounts(taskIds) {
  if (!taskIds.length) return {};
  const { data } = await supabase.rpc('task_comment_counts', { p_tasks: taskIds });
  return Object.fromEntries((data ?? []).map((r) => [r.task_id, Number(r.count)]));
}

export function canEditComment(c, userId) {
  return c.user_id === userId && Date.now() - new Date(c.created_at).getTime() < NOTE_EDIT_MINUTES * 60_000;
}