...
// ---------------- Visto (confirmación de lectura)
export async function getReads(messageIds) {
  if (!messageIds.length) return [];
  const { data } = await supabase.from('chat_message_reads').select('message_id, user_id, read_at').in('message_id', messageIds);
  return data ?? [];
}
...
export function useChatUnread(userId, { notify = true } = {}) {
  ...
  const load = useCallback(async () => {
    ...
    setUnread(n);
    // "(3) LeadForce": se ve aunque estés en otra pestaña
